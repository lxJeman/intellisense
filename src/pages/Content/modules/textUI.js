...
class TextUI {
  ...
  /**
   * Set language UI handler
   */
  setLanguageUI(languageUI) {
    this.languageUI = languageUI;
    console.log('🌐 Language UI connected to TextUI');
  }
  ...
  /**
   * Show language control panel for element
   */
  showLanguageControl(element, correctionData = null) {
    if (this.languageUI) {
      this.languageUI.showLanguageControl(element, correctionData);
    } else {
      console.warn('Language UI not available');
    }
  }
  ...
  showNotification(message, type = 'success') {
    const notification = document.createElement('div');
    notification.className = `intellisense-notification ${type}`;
    notification.textContent = message;
    document.body.appendChild(notification);
    setTimeout(() => notification.remove(), 3000);
  }
